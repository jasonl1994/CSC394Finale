  ...
  componentDidMount() {
	...
	fetch('/getUser').then(res => {
		res.json().then(data => {
			...
			StudentList.push("You have completed grouping students");
  ...
  render() {
    ...
    <Navbar.Brand href="#home" style = {{fontSize:'40px', color: 'lightblue', fontFamily: "'Rogue Script', cursive", shadow: '1px 1px 2px #000'}}>
      ...
      />{' Badge Buddy'}
		...
		<Form inline style = {{position: 'absolute', right: '0', width: '100px'}}>
	<Button variant="outline-light" type="submit" style = {{position: 'relative', zIndex:'99999999999999999999'}} onClick={this.handleLogout}>
						Log Out
	</Button>
	</Form>
  	</Navbar>
		...
		<Row style = {{marginRight:'15px'}}>
		  ...
		  <Col>
			...
			width: '800px', marginRight:'300px', marginTop:'100px'}}>
				  ...
				  <Card.Header>
					{this.state.Count - 2} student(s) left to group
